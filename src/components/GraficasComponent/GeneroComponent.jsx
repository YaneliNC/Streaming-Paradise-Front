  ...
  useEffect(() => {
    axios.get('https://streaming-paradise-server.onrender.com/usuarios-genero')
      .then(response => {
        ...
        const Masculino = Number(data.find(item => item.genero.toLowerCase() === 'masculino')?.cantidad) || 0;
        const Femenino = Number(data.find(item => item.genero.toLowerCase() === 'femenino')?.cantidad) || 0;

        setChartData(prevData => ({
          ...prevData,
          series: [Femenino, Masculino], // Asignamos los valores numéricos de femenino y masculino
        }));
      })
